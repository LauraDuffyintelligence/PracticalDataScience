...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income", label="choose salary", choices=c("<=50K","50K")),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId= "set_yaxis", label="choose yaxis", 
                  choose("Hours_per_week", "Capital_Loss")),
...
server <- function(input, output) {
  ...
  create_subset <- reactive(income %>% filter(capital_loss> 0 & 
                                                income==income$subset_income &
